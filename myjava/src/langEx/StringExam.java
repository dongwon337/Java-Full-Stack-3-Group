...
//		}else {
//			System.out.println("틀린 객체니, 값이 같은지는 모르겠음.");
//		}
//		//String의 override 된 equals 사용. 두 문자열 객체가 같은 값이면
//		//true 아님 false 리턴.
...
//		char[] chArr = {'오','늘','의','자','바','는','문','자','열','객','체','까','지','했','습','니','다'};
...
//		System.out.println(uniStr);
	 ...
	 * 
	 * 위 myMsg 를 byte 배열로 리턴하는 메서드가 있습니다. 리턴 타입이 byte[] 인
	 * 
	 * 애를 호출해서 바이트 배열로 변환후, 배열의 모든 값을 다시 새로운
	 * 문자열로 생성하는데, 단, 문자열의 캐릭터셋을 ISO8859 1 로 되어진 애로 변환해서
	 * 출력해보세요.
	 */

//		String myMsg = "오늘의 자바는 문자열 객체 까지 했습니다";
//		byte[] uniCodes = myMsg.getBytes();
//		System.out.println(Arrays.toString(uniCodes));
//		
//		String bStr = new String(uniCodes,0,uniCodes.length,"ISO-8859-1");
//		System.out.println(bStr);
...
//		//btStr 의 전체를 돌면서 문자(char) 를 리턴 받는다.
//		
...
//		
//		String hi = String.copyValueOf(new char[] {'h','e','l','l','o'});//hello 값을 갖는 char 배열을 파라미터에 완성시켜보세요.
//		
//		//문자열의 끝과 처음 판별하기
//		String email = "dongwon337@naver.com";
...
//		
//		String wSpace = "  h  ";
//		System.out.println(wSpace.length());
//		
//		int aa = 11;
//		
...
//		System.out.println(aaStr + 1);
		 ...
		 * 3_1 ID 와 서버로 분리하여 ID 를 검증(inValidate())하시는데
        ...
        System.out.println("ID(Email) 입력하세요");
        ...
        if(!(id.length() >= 8 && id.length() <= 12)) {
        	System.out.println("아이디는 8~12자 여야 합니다.");
        }else if(Character.isLowerCase(email.charAt(0)) == true) {
        	System.out.println("아이디의 첫글자는 대문자여야 합니다.");
        }else if(!(id.matches(".*[0-9].*"))) {
        	System.out.println("아이디에 반드시 숫자가 하나 이상 포함되어야 합니다.");
        }else {
        	System.out.println("로그인 되었습니다 " + email);
        }

	}
	static boolean isStringEmpty(String email) {
