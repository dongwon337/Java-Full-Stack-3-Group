 ...
 * 정의 하는데 있습니다.
 * 주의 해야 할 점은 이 로컬 클래스는 메서드에서만 존재하기 때문에 메서드가 실행이 되지 않을 경우엔
 * 존재 자체가 없습니다. 반대로 메서드가 실행이 끝난 경우에도 존재가 사라집니다.
 * 
 * 즉 외부에는 절대 노출이 안된다는 뜻입니다.
 * 
 * 이 로컬 클래스는 JDK 버전 없이 되기전에는 상수(final) 만 가질 수 있었는데, 아마 15 이상부터는
 * 상수 아니어도 지역 변수를 가질 수 있게 되었습니다. 어려우면 상수만 가질 수 있다라고 생각하세요.
 * 
 * 이 메서드는 앞에서 처럼 메서드 외부에 있는 다른 필드등(Inner 클래스 포함) 에 모두 접근 가능합니다.
 */
