 ...
 * 던지는 형태를 말합니다.
 * 
 * 이때 사용되는 형태는 대표적으로 알려진 or 실행 예외 두개정도를 만들어 쓸 수 있습니다.
 * 
...
//아래의 클래스는 Exception 을 상속받은 알려진 예외(Checked) 로 정의합니다.
//다른건 다 상속 받아서 부모의 메서드를 그대로 이용하면 되지만, 예외의 내용은 직접 정의해야 합니다.
//때문에 파라미터 한개의 생성자를 호출 후 예외 내용울 부모에게 전달하도록 합니다.
...
class Accout{
	...
	public int withdraw(int money) throws AccException{
		...
		if(balance < money) {
			throw new AccException("잔액이 부족 합니다." + (money - balance)
					+ "이 모자자랍니다.");
		}
