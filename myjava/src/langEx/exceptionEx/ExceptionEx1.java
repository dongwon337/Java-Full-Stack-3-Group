 ...
 * 해서 위 예외의 타입들은 컴파일시에 여러분이 예외 핸들링을 하지 않아도 컴파일러가
 * 관여하지 않습니다. 하지만 위 예외와 같은 계층이거나 상위에 있는 예외들은 알려진 예외로 컴파일러가
 * 관여를 하기에 여러분이 이런 API를 사용할떄는 예외를 핸들링(try~catch), 전파선언(throws),
...
public class ExceptionEx1 {
		...
		static void c() {
			...
			try {//예외 핸들링
			System.out.println(d / Integer.parseInt(input));
			
			System.out.println("여기 try 구문의 예외 발생후에 코드입니다.");
			}catch(Exception ae) {
				System.out.println("계산상의 예외 발생함..");
