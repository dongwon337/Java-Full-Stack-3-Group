...
public class VScopeExam {
	...
	public static void main(String[] args) {
		...
		//여기서는 자바에서 사용되는 변수의 scope에 대해 알아봅니다.
		//기본적으로는 메서드내에서 선언된 변수는 로컬 변수, 어떤 곳이던 {} 내에서 선언된
		//변수는 블락변수, 그리고 클래스에서 선언된 변수는 클래스 or 멤버필드로 지정되어집니다.
		//위, 클래스 or 멤버의 기준은 static 이라는 키워드가 좌우 합니다.
		//즉 static이 붙은 변수는 클래스 변수, none static은 멤버필드 (Instance 변수)라 합니다.
		//위 클래스 or 멤버필드는 순서대로 클래스가 메모리에 남아있는 동안 살아있고,
		//멤버는 객체가 몹쓸객체로 변이되기 전까지는 살아있습니다.
		//지역(local) 변수는 메서드의 body 가 끝나면 삭제되고, 블락은 블락이 실행후 삭제됩니다.
