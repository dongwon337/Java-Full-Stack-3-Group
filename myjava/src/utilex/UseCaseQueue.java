...
public class UseCaseQueue {
	...
	public static void main(String[] args) {
		System.out.println("help 를 입력 하면 도움말을 볼 수 있습니다.");
		try {
			while (true) {
				System.out.println(">>> ");
				...
				if (input.equalsIgnoreCase("q")) {
					...
					// Q에 있는 명령어를 추출하도록 정의 합니다.

					LinkedList<String> temp = (LinkedList<String>) q;
					ListIterator<String> it = temp.listIterator();
					// Iterator 타입 (내부순환자) 의 대표적 메서드로
					// 요소가 존재하면 true 리턴. 이걸 이용하면 요소의 끝까지
					// 리턴받을수있음.
					while (it.hasNext()) {
						// Iterator 의 next() 는 요소를 리턴받는 메서드입니다.
