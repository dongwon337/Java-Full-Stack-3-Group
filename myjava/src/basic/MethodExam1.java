...
public class MethodExam1 {
	//두개의 수를 입력 받아서 누가 더 큰 값인지를 리턴하는 메서드를 정의해봅니다.
	...
	static void print(String msg, int count) {
		 ...
		 * 정의 하고, 제공 하고 있습니다.
		 * 
		 * 메서드는 간단히 말하자면 자바스크립의 펑션, 우리가 어렸을 때 배운 함수와 같은 개념입니다.
		 * 
		 ...
		 * [static] : 클래스단위의 메서드인지의 여부 키워드..없으면 무조건 객체의 메서드로 선언한
		 * 것과 같음.
		 * return type : 정의한 메서드가 실행 중 또는 종료시에 넘겨줄 값이 존재한다면
		 * 해당 타입을 리턴으로 지정해야합니다. 만약 넘겨줄 것(return)이 없을경우엔
		 * void로 선언해야 합니다. 리턴타입에는 []를 쓰지 않았기 때문에 필수 입니다.
		 * 
		 ...
		 * 그냥 괄호만, 만약 필요시엔 해당 갑의 타입과, 그 값을 local에서 사용시 사용될 변수명을 넣어줍니다.
		 ...
		 * 리턴 시키면 됩니다. 로직 중간에 return이 호출되면 해당 메서드는 그 시점에 종료가 되어집니다.
