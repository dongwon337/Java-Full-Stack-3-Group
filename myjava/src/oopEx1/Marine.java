...
public class Marine extends Terran implements Attackable,Moveable {// 클래스에는 public or default 만 존재함.
	...
	/*
	 * 
	 * public Marine() {// 컴파일러가 넣어주는 기본 생성자가 어떤 역할을 하는지 코드로 정의합니다. super(); name =
	 * null; rank = null; hp = 0; damage = 0; }
	 */
	/*
	 * public Marine(String name) { this.name = name; }
	 */
	/*
	 * public Marine(String name, String rank) { /* 생성자 this([params]) : 이미 정의된 생성자를
	 * 호출하는 코드 파라미터는 옵션입니다. 이렇게 호출하면 정의된 생성자가 호출되어지기때문에 코드를 재사용 가능합니다. 단!!!!!!
	 * this() 는 무조건 생성자 첫 코드에서만 호출할 수 있고, 한번이상 사용안됩니다.
	 */
//		this.name = name;
//		this.rank = rank;
//	}
...
//	}
	@Override
	public void sound() {
		// implements 는 추상 메서드에 내용부 {} 를 정의한 형태를 말한다.
		// 만약 상속받은 메서드의 {}(body) 가 이미 존재한경우, 그 내용부만 변경할때
		// 재정의(override) 했다라고 하고, 그렇지 않은경우 (메서드가 선언만된경우)
		// 에 내용을 정의한 형채를 구현(implements) 헸다 라고 합니다.
	...
	public Marine(String name, String rank, int hp, int damage) {
		...
		/*
		 * 상속관계에서 자식 클래스에는 눈엔 보이지 않지만, 무조건 생성자 첫 머리에 컴파일러가 super() 를 호출하도록 되어있습니다.
		 */
		super("테란");
		this.name = name;
		this.rank = rank;
	...
	/*
	 * 부모에게 상속받은 메서드의 내용부만 재정의 해서 자식에 맞게 정의한 것을 객체지향에선 메서드 오버라이드(Override) 라고 합니다.
	 * 반드시 지켜야할 조건 : 접근제어자는 부모보다 같거나 커야하며, 리턴타입, 시그니쳐는 반드시 동일해야합니다.
	 */
	...
	public void attack(String target) {
		...
		System.out.println(super.getTheUnit() + "이(가)" + target + "총으로 공격합니다.");
