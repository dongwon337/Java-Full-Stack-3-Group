 ...
 * 쓰래드 자체의 메서드로 쓰래드를 제어 하는데 사용되는 대표적 메서드이다.
 * 
 * 참고로 쓰래드 자체의 매서드로 제어를 시키는데 사용되는 메서드는 아래와 같다.
 * 
...
public class ThreadJoinExam {
	...
	public static void main(String[] args) {
		...
		try {
			//join() 을 호출한 애는 main 쓰래드이고, 호출당한 애는 threadB 입니다.
			//따라서 threadB 가 run()을 모두 완료할때 main 쓰래드는 기다리는 상태가 됩니다.
			//정확히는 sleep()을 호출한 것과 같습니다.
			//이 때 main 쓰래드는 누가 깨워주지 않으면, 계속 실행대기 영역에서
			//머무르게 되기 때문에 문제가 발생합니다. 따라서 내부적으로 InterruptedException
