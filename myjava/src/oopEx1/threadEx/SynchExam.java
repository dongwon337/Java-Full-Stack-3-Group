 ...
 * 공유된 멤버필드를 사용할 때 동기화가 되어지지 않으면, 올바른 값이 도출되지 않을 수 있다.
 * 해서 자바에서는 동기화란 개념을 통해 하나의 쓰래드가 공유 객체를 점유 하는 순간 해당 객체에
 * lock을 걸어서 다른 쓰래드가 접근을 할 수 없도록 한다. 점유한 쓰래드가 작업을 다 마칠때까지
 * 다른 쓰래드는 대기 영역에 머물게 되고, lock 을 해제하는 순간 객체에 접근 하여 run() 을 수행한다.
 * 
...
class Account{
	...
	public synchronized void withdraw(int money) {//인출 메서드
...
class ConThread implements Runnable{
	//이 쓰래드가 위의 Account 객체 하나를 공유 하도록 하기 위해
