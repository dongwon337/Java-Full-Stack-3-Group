 ...
 * 만을 위한것으로 생각될 때, 해당 메소드를 추상화 할 수 있습니다.
 * 이렇게 추상화된 메서두를 추상메서드라 하고, 이 메서드는 abstract 라는 키워드를
 * 통해 선언을 하게됩니다.
 * 
...
public abstract class Terran extends Object{
	 ...
	 * 내 어디에도 사용될 수 있습니다.
	 * 
	 * 이름에서도 알수 있듯이 , final 키워드뭔가를 고정시키는 역할을 합니다.
	 * 클래스에 붙으면 상속금지, 메서드에 붙으면 오버라이드 금지, 필드에 붙으면 고정값(상수화)
	 * 메서드 파라미터에 붙으면 메서드 내부에서 상수화 의미로 쓰입니다.
	 * 
	 * 일반적으로 자바에서는 상수로 사용될땐 공유의 목적으로 static 과 같이 사용되어집니다.
	 * 
	 */
	
	
	
//	//static final 필드는 반드시 초기화 합니다.
//	final static int ALL_UNIT_MIN_HP = 0;
//	final int MARINE_MAX_HP = 40;
//	final int FIREBET_MAX_HP = 40;
...
//	
	...
	//추상메서드는 abstract 키워드와 함께 선언만 할수잇음
