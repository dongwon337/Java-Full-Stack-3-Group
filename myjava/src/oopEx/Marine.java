...
//클래스에는 크게 3가지를 정의할 수 있는데, 속성, 생성자, 메서드 입니다.
...
public class Marine extends Terran implements Attackable, Moveable{//클래스에는 public or default 만 존재함.
	...
	//속성정의.//Fields --> 객체를 구성하는 속성..즉 객체 속성 멤버필드..
	 ...
	 * 안되고(하면 메서드화 되버림), 생성자 명은 반드시 클래스명과 같아야 한다는 점입니다.
	 * 만약 여러분이 생성자를 하나도 정의 하지 않은경우엔 컴파일시 컴파일러가 기본생성자(default)
	 * 를 자동으로 클래스내에 넣어줍니다. 하지만 여러분이 하나라도 정의하면 컴파일러는 제공하지
	 * 않습니다.
	 * 
	...
	/*생성자 오버로딩*/
	/*
	public Marine() {//컴파일러가 넣어주는 기본 생성자가 어떤 역할을 하는지 코드로 정의합니다.
		super();
		name = null;
		rank = null;
		hp = 0;
...
//	public Marine(String name, String rank) {
		 ...
		 * 파라미터는 옵션입니다. 이렇게 호출하면 정의된 생성자가 호출되어지기때문에
		 * 코드를 재사용 가능합니다.
...
//	}
	...
	public void sound() {
		...
		//재정의(Override)했다 라고 하고, 그렇지 않은 경우(메서드가 선언만 된경우)
	...
	public void attack(String target) {
		System.out.println(super.getTheUnit() + " 이 총으로 " + target + " 을 공격합니다.");
	}
